    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 32px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <button
            onClick={() => navigate('/')}
            style={{ background: 'transparent', border: 'none', color: 'var(--muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, padding: 0 }}
          >
        ...
        <button
          ...
          style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 6, fontSize: '0.8rem' }}
          onMouseEnter={e => { e.currentTarget.style.color = 'var(--text)'; e.currentTarget.style.borderColor = '#334155'; }}
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '40px 24px' }}>
        ...
        {error && (
          <div style={{ background: '#450a0a', border: '1px solid var(--danger)', borderRadius: 6, padding: '10px 16px', marginBottom: 20, color: '#fca5a5', fontSize: '0.875rem' }}>
            {error}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
            {locations.map(loc => (
              <div
                key={loc.id}
                style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 10 }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <MapPin size={17} color="var(--success)" />
      ...
      {creating && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#1e293b', padding: 28, borderRadius: 10, color: '#f8fafc', width: 340, border: '1px solid #334155' }}>
            ...
            <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: 4 }}>Name <span style={{ color: 'var(--danger)' }}>*</span></label>
            <input
              autoFocus
              value={newName}
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
              ...
              <button style={{ padding: '7px 18px', background: 'var(--accent)', border: 'none', color: '#fff', borderRadius: 6, cursor: 'pointer' }} onClick={handleCreate}>Create</button>
            </div>
          </div>
        </div>
      )}
