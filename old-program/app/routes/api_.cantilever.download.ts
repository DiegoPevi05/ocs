...
import { getCantileversByTypeAndValue } from "~/db/cantilever/actions.server";
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const type = url.searchParams.get("type") as "external_id" | "via" | "location" | "pole" | null;
  ...
  const cantileverDataArray = await getCantileversByTypeAndValue(request, projectId, type, value, pov);
  if (!cantileverDataArray) {
    return Response.json({ error: "No cantilevers found" }, { status: 404 });
  }

  const projectName = cantileverDataArray[0].cantilever.project;
  ...
  if(cantileverDataArray.length > 0){

    let cantileversContent = generateCantileversHtmlReport(
      t,
      cantileverDataArray[0].cantilever.location,
      cantileverDataArray[0].cantilever.via,
      cantileverDataArray[0].cantilever.pole,
      cantileverDataArray
    );
