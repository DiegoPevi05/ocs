...
import {  addLineToVia } from "~/db/via/actions.server";
...
export const action: ActionFunction = async ({ request }) => {
  ...
  switch (request.method) {
    case "POST": {
      ...
      const newSegments     = await request.json();
      const updatedVia      = await addLineToVia(request, viaId, projectId, newSegments);
